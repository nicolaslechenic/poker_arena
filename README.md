...
Poker Arena uses a simple JSON-based NoSQL storage system for persistence:

### JSON Storage

The application stores data in JSON files located in the `data` directory:

- `data/players.json`: Stores player information including tokens, pseudos, and bankrolls
- `data/hand_histories.json`: Stores complete hand histories including actions, cards, and results

This lightweight NoSQL approach provides:

1. **Simplicity**: Easy to understand and maintain without complex database setup
2. **Persistence**: Data survives application restarts
3. **Human-readable**: JSON files can be easily inspected and modified if needed
4. **Portability**: No external database dependencies required

### Implementation Details

The storage system consists of:

- `JsonStore`: A generic JSON file-based storage class
- `PlayerSerializer`: Converts Player objects to/from JSON
- `HandHistorySerializer`: Converts HandHistory objects to/from JSON
- Updated repositories that use these components

To access hand history data, use the following endpoints:

```
GET /api/hand_histories/:id
```

```
GET /api/tables/:name/hand_histories
```

This provides a complete record of all poker hands played on the platform.
