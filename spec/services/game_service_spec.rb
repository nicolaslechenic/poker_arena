...
RSpec.describe PokerArena::Services::GameService do
  ...
  describe '#player_folded?' do
    ...
    context 'when player has folded' do
      before do
        action = PokerArena::Action.new(player: player1, type: :fold, value: 0)
  ...
  describe '#player_bet' do
    ...
    context 'when a player makes additional bets' do
      before do
        action = PokerArena::Action.new(player: player1, type: :call, value: table.big_blind)
        game.add_action(action)
      end
  ...
  describe '#current_bet' do
    ...
    context 'when a player raises' do
      before do
        action = PokerArena::Action.new(player: player1, type: :raise, value: table.big_blind * 2)
        game.add_action(action)
      end
