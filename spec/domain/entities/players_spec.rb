...
RSpec.describe PokerArena::Domain::Entities::Player do
  describe '#receive_card' do
    it 'raise an error for full of card player' do
      player = described_class.new(pseudo: 'Jon Snow')

      %w[A K].each do |litteral|
        player.receive_card(PokerArena::Domain::Entities::Card.x(litteral))
      end

      expect do
        player.receive_card(PokerArena::Domain::Entities::Card.x('Q'))
